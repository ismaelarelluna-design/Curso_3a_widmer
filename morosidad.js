...
function downloadVoucherPDF() {
    ...
    // La descarga automática por blob (html2pdf().save()) la bloquean muchos
    // navegadores móviles de forma silenciosa (sin error, sin descarga). Por
    // eso, sin importar si la app está instalada o abierta en el navegador,
    // SIEMPRE abrimos una pestaña vacía YA MISMO (dentro del clic, para que
    // no la bloquee el navegador) y la llenamos con el PDF cuando esté listo.
    ...
    html2pdf().set(opt).from(clone).outputPdf('datauristring').then(dataUri => {
        if (pdfWindow) {
            ...
            // Se bloqueó la ventana emergente: intentamos la descarga directa como último recurso.
